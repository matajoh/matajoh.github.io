...
/** TODO
- Add the pre-computed weights and offsets for the different gaussian blur kernels that are needed
- Compute the blurs and place them in a single shared texture
- Compute the DoGs from the shared texture to create a second texture (this one containing the DoG response)
- Compute the keypoints from the DoG response texture
